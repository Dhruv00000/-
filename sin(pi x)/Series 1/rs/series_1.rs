...
fn main() {
    ...
    loop {
        ...
        approximation += i8::pow(-1, iterator as u32) as f64 / (iterator as f64 + 1.0/2.0); // The series was originally a summation over all integers, so I split it into positive and neagtive branches.
        if iterator != 0 { approximation += i8::pow(-1, iterator as u32) as f64 / (1.0/2.0 - iterator as f64); } // I'm checking here for k != 0 because without it, zero would be counted in both branches.
