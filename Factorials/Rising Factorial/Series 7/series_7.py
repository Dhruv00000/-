...
while True:
    ...
    approximationInverted += Decimal((8*k + 1) * risingFactorial(1/2, k) * risingFactorial(1/4, k) * risingFactorial(3/4, k) / (pow(factorial(k), 3) * pow(9, k)))
    approximation = 2 * Decimal(pow(3, 1/2)) / approximationInverted
